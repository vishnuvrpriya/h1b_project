...
show_menu()
{
    ...
    echo -e "${MENU}${NUMBER} 3) ${MENU} Which industry has the most number of Data Scientist positions?(HIVE)${NORMAL}"
    echo -e "${MENU}${NUMBER} 4) ${MENU} Which top 5 employers file the most petitions each year?(MR) ${NORMAL}"
    echo -e "${MENU}${NUMBER} 5A) ${MENU} Find the most popular top 10 job positions for H1B visa applications for each year?(PIG)${NORMAL}"
    echo -e "${MENU}${NUMBER} 5B) ${MENU} Find the most popular top 10 job positions for Certified H1B visa applications for each year?(HIVE)${NORMAL}"
    echo -e "${MENU}${NUMBER} 6) ${MENU} Find the percentage and the count of each case status on total applications for each year. Create a graph depicting the pattern of All the cases over the period of time.(PIG)${NORMAL}"
    echo -e "${MENU}${NUMBER} 7) ${MENU} Create a bar graph to depict the number of applications for each year(HIVE)${NORMAL}"
    ...
    echo -e "${MENU}${NUMBER} 9) ${MENU}Which are the employers who have highest success rate in petitions more than 70%and total petions filed more than 1000?(PIG)${NORMAL}"
    echo -e "${MENU}${NUMBER} 10) ${MENU}Which are the top 10 job positions which have the  success rate more than 70% in petitions and total petitions filed more than 1000? (PIG)${NORMAL}"
